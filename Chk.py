...
async def chk_command(message: types.Message):
    ...
    if not await asyncio.to_thread(is_gate_enabled, "chk"):
        await message.reply(
            "<tg-emoji emoji-id='4958926882994127612'>🚧</tg-emoji> <b>𝗦𝘁𝗿𝗶𝗽𝗲 𝟬$ 𝗔𝘂𝘁𝗵 𝗶𝘀 𝘂𝗻𝗱𝗲𝗿 𝗺𝗮𝗶𝗻𝘁𝗲𝗻𝗮𝗻𝗰𝗲.</b>\n"
    ...
    if not raw_text:
        await message.reply(
            "<tg-emoji emoji-id='5040030395416969985'>🚫</tg-emoji> <b>Usage:</b> /chk <code>cc|mm|yy|cvv</code>\n"
    ...
    if not match:
        await message.reply(
            "<tg-emoji emoji-id='4915853119839011973'>⚠️</tg-emoji> <b>Invalid Card Format.</b>\n"
...
async def process_chk_check(message, proc_msg, user, user_id, formatted_cc, cc, plan_name):
    ...
    if "APPROVED" in status_raw:
        ...
        final_status = "𝗘𝗥𝗥𝗢𝗥 / 𝗧𝗜𝗠𝗘𝗢𝗨𝗧 <tg-emoji emoji-id='6237864166879663987'>❌</tg-emoji>"
    ...
    final_caption = (
        ...
        f"<b><tg-emoji emoji-id='5039895103947146186'>🌐</tg-emoji> 𝗚𝗮𝘁𝗲:</b> {to_math_bold("Stripe 0$")}\n"
