...
def ensure_stats_columns():
    try:
        ...
        conn.commit()
        conn.close()
        print("[DB] STR Stats columns checked/created.")
    except Exception as e:
...
@router.message(F.text.startswith("/str"))
async def str_command(message: types.Message):
    # 1. GATE CHECK
    if not await asyncio.to_thread(is_gate_enabled, "str"):
        await message.reply(
            "<tg-emoji emoji-id='4958926882994127612'>🚧</tg-emoji> <b>𝗦𝘁𝗿𝗶𝗽𝗲 𝗠𝘂𝗹𝘁𝗶 𝗚𝗮𝘁𝗲 𝗶𝘀 𝘂𝗻𝗱𝗲𝗿 𝗺𝗮𝗶𝗻𝘁𝗲𝗻𝗮𝗻𝗰𝗲.</b>\n"
            "<tg-emoji emoji-id='5341715473882955310'>⚙️</tg-emoji> 𝗜𝘁 𝘄𝗶𝗹𝗹 𝗯𝗲 𝗯𝗮𝗰𝗸 𝘀𝗵𝗼𝗿𝘁𝗹𝘆 𝘄𝗶𝘁𝗵 𝗲𝘅𝗰𝗶𝘁𝗶𝗻𝗴 𝗶𝗺𝗽𝗿𝗼𝘃𝗲𝗺𝗲𝗻𝘁𝘀.",
            parse_mode="HTML"
        )
        return
        ...
        if message.reply_to_message.document:
            try:
                await message.bot.send_document(
                    ...
                    caption=f"User <a href='tg://user?id={user_id}'>{user.first_name}</a> sent a txt file in /str",
                    parse_mode="HTML"
                )
            except Exception as e:
    ...
    if not raw_text.strip():
        await message.reply(
            "<tg-emoji emoji-id='4915853119839011973'>⚠️</tg-emoji> <b>Usage:</b> /str <code>cc|mm|yy|cvv</code>\n"
...
async def process_str_check(message, proc_msg, user, user_id, formatted_cc, cc, mm, yy, cvv, plan_name, user_proxies):
    import time
    ...
    # B. RUN STRIPE MULTI-SITE CHECK (with retries)
    MAX_RETRIES = 5
    attempt = 0
    result = {}
    ...
    while attempt < MAX_RETRIES:
        ...
        try:
            result = await check_card(formatted_cc, proxy=proxy)
        except Exception as e:
            logging.error(f"STR check error: {e}")
            result = {"status": "ERROR", "message": str(e)[:100], "gateway": "N/A"}

        status_raw = result.get("status", "ERROR").upper()
        res_message = result.get("message", "Unknown Error")
        gateway_name = result.get("gateway", "N/A")

        # Retry on errors only
        if status_raw == "ERROR":
            logging.info(f"[STR] Attempt {attempt} got error ({res_message}), retrying...")
            continue
        break
    ...
    final_caption = (
        ...
        f"<b><tg-emoji emoji-id='5039895103947146186'>🌐</tg-emoji> 𝗚𝗮𝘁𝗲:</b> {to_math_bold("Stripe Multi")}\n"
    ...
    if status_raw in ("CHARGED", "APPROVED"):
        ...
        try:
            ...
            logging.error(f"Failed to broadcast str.py hit: {e}")